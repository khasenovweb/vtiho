$(document).ready(function () {
  ...
  function calc() {
    ...
    if ($('.calc__form input[name="montazh"]').prop("checked") === true) {
      var percent = $('.calc__form input[name="montazh"]').attr("data-percent");
  ...
  $(".cart__modal__wrapper").click(function () {
    if ($(event.target).closest(".cart__modal").length) return;
    $(this).hide();
    $("body").removeClass("hidden");
  });
  ...
  function map_read_all() {
    $(".mappanel__content1__readall").click(function () {
      var index = $(this).index();
      $(".mappanel__content1__text")
        .eq(index)
        .toggleClass("mappanel__content1__text__all");
      $(this).css("opacity", "0");
    });
  }
  map_read_all();
